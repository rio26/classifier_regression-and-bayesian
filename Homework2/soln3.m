...
%% 2.1 Maximum A Posteriori estimate (with Naive assumption)
...
for c = 1:N     
    ...
    M_D_naive(c) = (1/sqrt((2*pi)^2 * det(sigma_D)))*exp(-0.5*diff_D'*inv(sigma_D)*diff_D)* classPrior_D;
end
...
for c = 1:N
    ...
    M_H_naive(c) = (1/sqrt((2*pi)^2 * det(sigma_H)))*exp(-0.5*diff_H'*inv(sigma_H)*diff_H)* classPrior_H;
end

for c = 1:N
	if M_D_naive(c) >= M_H_naive(c)
		prediction_MPnaive(c) = 1; %label diseased
	else
		prediction_MPnaive(c) = 0; %label health
	end
end
%% 2.1 Plot the data and predictions
color_train = {'ro','bo'};
color_test = {'rx','bx'};
figure(1);
hold on
for c = 1:N
    if prediction_MPnaive(c) == 1;
...
sum(prediction_MPnaive == 1)
sum(prediction_MPnaive == 0)

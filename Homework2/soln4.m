...
%% 2.2 Maximum A Posteriori estimate (without Naive assumption)
sigma_D = class_varD; 
for c = 1:N
    diff_D = newpts_test(:,c)-class_meanD;
    M_D_notnaive(c) = (1/sqrt((2*pi)^2 * det(sigma_D)))*exp(-0.5*diff_D'*inv(sigma_D)*diff_D)* classPrior_D;
end

sigma_H = class_varH; 
for c = 1:N
    diff_H = newpts_test(:,c)-class_meanH;
    M_H_notnaive(c) = (1/sqrt((2*pi)^2 * det(sigma_H)))*exp(-0.5*diff_H'*inv(sigma_H)*diff_H)* classPrior_H;
end

% assign labels to the points
for c = 1:N
	if M_D_notnaive(c) >= M_H_notnaive(c)
		prediction_MP_notnaive(c) = 1; %label diseased
	else
		prediction_MP_notnaive(c) = 0; %label health
	end
end
%% 2.2 Plot the data and predictions for Maximum Likelihood(without Naive assumption)
color_train = {'ro','bo'};
color_test = {'rx','bx'};
figure(2);
hold on
for c = 1:N
    if prediction_MP_notnaive(c) == 1;
...
sum(prediction_MP_notnaive == 1)
sum(prediction_MP_notnaive == 0)
